...
const  float kHeaderView_X =0.0f; //顶部开始坐标X
const  float kHeaderView_Y =0.0f; //顶部开始坐标Y
const  float kHeaderView_Width =320.0f;//顶部宽度
...
- (void)reloadData
{
    ...
    if ([delegate respondsToSelector:@selector(headerForStreamView:)]) {
        ...
        if (tempHeaderView) {
            ...
            f.origin = CGPointMake(kHeaderView_X, kHeaderView_X);
            f.size.width = kHeaderView_Width;
